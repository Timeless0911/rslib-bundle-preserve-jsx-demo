...
- `main`: the normal library entry. It can bundle normally, but marks `preserve-jsx-dep1` and `preserve-jsx-dep2` as external and maps them to their companion vendor package folders.
- `vendor-jsx-dep1` and `vendor-jsx-dep2`: isolated bundleless outputs for each preserve JSX dependency. They use `bundle: false` plus React `runtime: 'preserve'`, so JSX syntax is preserved in the generated vendor files.
...
- `dist/index.js`: imports from `./vendor/preserve-jsx-dep1/index.jsx` and `./vendor/preserve-jsx-dep2/index.jsx`
